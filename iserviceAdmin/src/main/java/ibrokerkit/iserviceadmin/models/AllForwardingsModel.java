...
public class AllForwardingsModel extends LoadableDetachableModel implements IChoiceRenderer {

	private static final long serialVersionUID = -605535688450064767L;

	private static Log log = LogFactory.getLog(AllForwardingsModel.class.getName());
	...
	public Object load() {
		...
		log.debug("Listing all forwardings.");

		try {

			list = Arrays.asList(iservice4javaStore.listForwardings());
		} catch (Exception ex) {
	...
	public String getIdValue(Object object, int index) {
		...
		return(((Forwarding) object).getId().toString());
	}
}
