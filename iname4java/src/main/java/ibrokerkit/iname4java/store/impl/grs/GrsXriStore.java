package ibrokerkit.iname4java.store.impl.grs;
...
import ibrokerkit.iname4java.store.impl.openxri.OpenxriXri;
...
public class GrsXriStore extends OpenxriXriStore {
	...
	public GrsXriStore(Store openxriStore, EppTools eppTools) {
	...
	public Xri transferAuthorityInRequest(String localName, XriData xriData) throws XriStoreException {
		...
		try {
			...
			for (String iname : inames) {
				...
				SubSegment subSegment = this.openxriStore.registerSubsegment(null, iname, newXri.getAuthority());
				...
				((StoreAttributable) this.openxriStore).setSubSegmentIndex(subSegment, grsXriData.getUserIdentifier());
			}
		} catch (StoreException ex) {
