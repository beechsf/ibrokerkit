package ibrokerkit.iservicefront.forwarding.webpages;
...
public abstract class ForwardingBasePage extends WebPage {

	private static final long serialVersionUID = -2014848162732407794L;

	protected HashMap<String, Object> velocityMap;

	public ForwardingBasePage() {
	...
	protected void addVelocity(final MyVelocityPanel subVelocityPanel) {
		...
		this.add(new MyVelocityContributor(Model.of(this.velocityMap)) {
			...
			protected Reader getTemplateReader() throws IOException {
				...
				String file = "velocity/forwarding-head.vm";
				String path = ((WebApplication) ForwardingBasePage.this.getApplication()).getWicketFilter().getFilterConfig().getServletContext().getRealPath(file);
		...
		this.add(new MyVelocityPanel("velocity", Model.of(this.velocityMap)) {
			...
			protected String getFilename() {
				...
				return("velocity/forwarding-base.vm");
			}
		});
	}
}
