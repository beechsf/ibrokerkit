...
public class ForwardingJob extends Job {

	private String xri;
	
	public ForwardingJob(String xri) {
	...
	public void execute(Resolver resolver) throws PartialResolutionException, IllegalTrustTypeException {
		...
		List<?> uris = resolver.resolveSEPToURIList(
				new XRI(this.xri), 
				ForwardingService.SERVICE_TYPE, 
				null, 
				flags,
				state);
		...
		this.result.append("Forwarding Service endpoint(s) for " + this.xri + ":<br>");
		...
		this.oneLineResult.append("Forwarding Service endpoint(s) for " + this.xri + ": ");
